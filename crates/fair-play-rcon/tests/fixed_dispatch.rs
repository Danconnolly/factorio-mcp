...
fn contract_response() -> String {
    r#"{"ok":true,"result":{"schema_version":"0.1.0","fair_play_policy_version":"0.1.0","bridge_build":"0.1.0","factorio_build":"2.0.77","enabled_mods":[],"actor_id":"alfred","force_id":"player","profile_name":"phase-zero-read-only","observation_limits":{"max_radius":32,"max_result_count":128,"max_detail_fields":4,"max_payload_bytes":32768},"scheduling":{"execution":"read_only","observation_tick":"bridge_tick_snapshot","request_order":"serialized"},"capabilities":["get_capability_contract","get_actor_status","scan_local","scan_charted","get_entity","get_action_record"],"supported_provenance":["character_local"],"current_game_tick":42}}"#.to_owned()
}
...
async fn bridge_version_mismatch_fails_closed() {
    let mismatched =
        contract_response().replace("\"bridge_build\":\"0.1.0\"", "\"bridge_build\":\"0.1.1\"");
