...
impl AuditSink {
    ...
    pub fn append(&self, record: &AuditRecord) -> Result<AuditEnvelope, ContractError> {
        ...
        let result = (|| {
            ...
            let line = fair_play_contract::canonical_json(&envelope)?;
...
impl PhaseZeroServer {
    ...
    fn result<T: serde::Serialize>(value: &T) -> CallToolResult {
        match fair_play_contract::canonical_json(value) {
...
impl PhaseZeroServer {
    ...
    async fn get_action_record(
        ...
        if input.sequence > fair_play_rcon::MAX_LUA_SAFE_INTEGER {
...
#[tool_handler(router = self.tool_router, name = "fair-play-mcp", version = "0.1.0", instructions = "Phase-0 read-only Factorio capability profile. All observations are bounded and policy-labelled.")]
impl ServerHandler for PhaseZeroServer {
    fn get_info(&self) -> rmcp::model::ServerInfo {
        let contract = fair_play_contract::canonical_json(&self.contract)
        ...
        .with_server_info(rmcp::model::Implementation::new("fair-play-mcp", "0.1.0"))
...
fn observation_record(
    ...
    result: &fair_play_rcon::ObservationResult,
) -> AuditRecord {
    AuditRecord::Observation(ObservationRecord {
        capability,
        game_tick: 0,
...
fn error_result(error: ContractError) -> CallToolResult {
    let text = fair_play_contract::canonical_json(&error).unwrap_or_else(|_| {
...
mod tests {
    use super::*;
    use fair_play_contract::{AuditChain, AuditRecord, Provenance};
