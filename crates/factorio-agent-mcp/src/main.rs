...
use factorio_agent_mcp::{AuditSink, PhaseZeroServer};
use factorio_agent_rcon::{RconAdapter, RconConfig};
use rmcp::ServiceExt;
...
async fn main() -> ExitCode {
    ...
    let audit_path = env::var_os("FAIR_PLAY_AUDIT_LOG").map_or_else(
        || PathBuf::from("factorio-agent-mcp-audit.jsonl"),
        PathBuf::from,
    );
    ...
    let contract = match bridge
        .call(factorio_agent_rcon::BridgeCommand::GetCapabilityContract)
        .await
    {
        Ok(factorio_agent_rcon::BridgeResponse::CapabilityContract(contract)) => contract,
        Ok(_) => {
