...
`fair-play-contract` is transport-independent. A bridge or MCP transport must first
...
```sh
cargo run -p fair-play-contract --example generate_schemas
cargo test -p fair-play-contract --test schema_contract -- --nocapture
```
