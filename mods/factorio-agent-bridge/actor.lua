...
local function marker_for(actor_id)
  return "factorio-agent-bridge:" .. actor_id .. ":" .. config.POLICY_VERSION
end
...
local function ensure_record()
  ...
  if state.actor and state.actor.actor_id ~= configured_actor_id then
    error("factorio-agent-bridge: startup actor ID cannot change after initialization")
  end
...
function actor.validate_after_load()
  if storage.factorio_agent_bridge and storage.factorio_agent_bridge.actor then
    actor.ensure()
  end
end
